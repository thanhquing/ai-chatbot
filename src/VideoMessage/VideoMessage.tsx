...
const VideoMessage: React.FC<IVideoMessageProps> = props => {
  ...
  return (
    <div
      className={classNames('HD_aichatbot-mbox-video', {
        'padding-time': !props?.text,
      })}
    >
      <div
        className='HD_aichatbot-mbox-video--video'
        style={{
          ...(props?.data.width &&
            props?.data.height && {
      ...
      >
        ...
        {error && (
          <div className='HD_aichatbot-mbox-video--video__block'>
            <span className='HD_aichatbot-mbox-video--video__block-item HD_aichatbot-mbox-video--error'>
        ...
        {!error && props?.data?.status && !downloaded && (
          <div className='HD_aichatbot-mbox-video--video__block'>
            {!props.data.status.click && (
              <button onClick={props.onDownload} className='HD_aichatbot-mbox-video--video__block-item HD_aichatbot-mbox-video--download'>
                <FaCloudDownloadAlt />
              </button>
            )}
            {typeof props.data.status.loading === 'number' && props.data.status.loading !== 0 && (
              <ProgressCircle
                ...
                className='HD_aichatbot-mbox-video--video__block-item'
      ...
      {props?.text && <div className='HD_aichatbot-mbox-text'>{props.text}</div>}
    </div>
  )
}
