...
const PhotoMessage: React.FC<IPhotoMessageProps> = props => {
  ...
  return (
    <div className='HD_aichatbot-mbox-photo'>
      <div
        className='HD_aichatbot-mbox-photo--img'
        style={{
          ...(props?.data?.width &&
            props?.data?.height && {
      ...
      >
        ...
        />
        {error && (
          <div className='HD_aichatbot-mbox-photo--img__block'>
            <span className='HD_aichatbot-mbox-photo--img__block-item HD_aichatbot-mbox-photo--error'>
        ...
        {!error && props?.data?.status && !props?.data?.status?.download && (
          <div className='HD_aichatbot-mbox-photo--img__block'>
            {!props?.data?.status.click && (
              <button onClick={props.onDownload} className='HD_aichatbot-mbox-photo--img__block-item HD_aichatbot-mbox-photo--download'>
                <FaCloudDownloadAlt />
              </button>
            )}
            {typeof props?.data?.status.loading === 'number' && props?.data?.status.loading !== 0 && (
              <ProgressCircle
                ...
                className='HD_aichatbot-mbox-photo--img__block-item'
              />
            )}
          </div>
        )}
