...
const Avatar: React.FC<IAvatarProps> = ({ type = 'default', size = 'default', lazyLoadingImage = undefined, ...props }) => {
  ...
  return (
    // @ts-ignore
    <div className={classNames('HD_aichatbot-avatar-container', type, size, props.className)}>
      {props.letterItem ? (
        <div className='HD_aichatbot-avatar-letter-background' style={{ backgroundColor: stringToColour(props.letterItem.id) }}>
          <span className='HD_aichatbot-avatar-letter'>{props.letterItem.letter}</span>
        </div>
      ) : (
        <img
          alt={props.alt}
          ...
          className={classNames('HD_aichatbot-avatar', { 'HD_aichatbot-avatar-lazy': isLazyImage })}
        />
      )}
      {props.sideElement}
    </div>
